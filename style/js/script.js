$(document).ready(function () {
	...
	$("#confirmacaoSenha, #senha_usuario").change(function () {
		if (
			$("#confirmacaoSenha").val() != $("#senha_usuario").val() &&
			$("#confirmacaoSenha").val() != "" &&
			$("#senha_usuario").val() != ""
		) {
			toast("Senha", "Senhas não conferem!", "error");
			$("#confirmacaoSenha, #senha_usuario")
				.closest(".form-group")
		...
		if (e.keyCode != 9) {
			...
			if ($(this).val() == "" || $(this).val() == "Selecione...") {
				...
				if (!$(this).hasClass("mascara_data")) {
					toast(
						"Campo em Branco",
						"Campo " + aviso + " deve ser preenchido!",
						"error"
					);
					$(this).closest(".form-group").removeClass("has-success");
					$(this).closest(".form-group").addClass("has-error");
				...
				if (
					!$(this).hasClass("validar_cnpj") &&
					!$(this).hasClass("validar_cpf") &&
					!$(this).hasClass("validar_email")
				) {
					$(this).closest(".form-group").removeClass("has-error");
					$(this).closest(".form-group").addClass("has-success");
				}
	...
	$("#validar_Enviar").click(function (e) {
		...
		$(".obrigatorio").each(function () {
			...
			if ($(this).val() == "" || $(this).val() == "Selecione...") {
				...
				if (mensagem == "") {
					//Garante que não irá se repetir.
					mensagem =
						"O(s) Campo(s): " +
						"<li><strong>" +
						aviso.toUpperCase() +
					...
					mensagem =
						mensagem + "<li><strong>" + aviso.toUpperCase() + "</strong></li>";
				}
		...
		if (erro_cnpj || erro_cpf || erro_email) {
			//Garante que não existem erros na tela

			swal(
				"Campo(s) em Branco",
				"Existe(m) campo(s) com erro(s) de preenchimento!",
				"error"
			);
